  ...
  sub count {
    ...
    for my $x ($x0 - 1 .. $x0 + 1) {
      for my $y ($y0 - 1 .. $y0 + 1) {
        for my $z ($z0 - 1 .. $z0 + 1) {
          for my $w ($w0 - 1 .. $w0 + 1) {
            next if ($x == $x0 && $y == $y0 && $z == $z0 && $w == $w0);
            next unless ($self->{ map }{ "$x,$y,$z,$w" });
            $count++;
           }
         }
       }
    ...
    for my $x ($self->{ min }[0] - 1 .. $self->{ max }[0] + 1) {
      for my $y ($self->{ min }[1] - 1 .. $self->{ max }[1] + 1) {
        for my $z ($self->{ min }[2] - 1 .. $self->{ max }[2] + 1) {
          for my $w ($self->{ min }[3] - 1 .. $self->{ max }[3] + 1) {
            my $count = $self->count( $x, $y, $z, $w );
            if ($self->{ map }{ "$x,$y,$z,$w" } && $self->{ map }{ "$x,$y,$z,$w" } eq '#') {
              $new->set( $x, $y, $z, $w ) if ($count == 2 || $count == 3);
             }
            else {
              $new->set( $x, $y, $z, $w ) if ($count == 3);
             }
           }
         }
       }
